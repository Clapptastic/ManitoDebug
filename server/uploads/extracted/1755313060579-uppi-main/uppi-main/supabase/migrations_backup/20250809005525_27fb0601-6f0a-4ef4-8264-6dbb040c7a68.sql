-- Prompts & Versions schema with RLS, triggers, and policies (super_admin-only write; service_role read-only)
...
-- 5) Policies
-- READ: super_admin and service_role may read
create policy if not exists prompts_select_super_admin_or_service
on public.prompts for select
using (
  public.get_user_role(auth.uid()) = 'super_admin' or auth.role() = 'service_role'
);

create policy if not exists prompt_versions_select_super_admin_or_service
on public.prompt_versions for select
using (
  public.get_user_role(auth.uid()) = 'super_admin' or auth.role() = 'service_role'
);

-- WRITE: only super_admin may insert/update/delete
create policy if not exists prompts_insert_super_admin
on public.prompts for insert
with check (
  public.get_user_role(auth.uid()) = 'super_admin'
);

create policy if not exists prompts_update_super_admin
...
create policy if not exists prompts_delete_super_admin
on public.prompts for delete
using (
  public.get_user_role(auth.uid()) = 'super_admin'
);

create policy if not exists prompt_versions_insert_super_admin
on public.prompt_versions for insert
with check (
  public.get_user_role(auth.uid()) = 'super_admin'
);

create policy if not exists prompt_versions_update_super_admin
...
create policy if not exists prompt_versions_delete_super_admin
