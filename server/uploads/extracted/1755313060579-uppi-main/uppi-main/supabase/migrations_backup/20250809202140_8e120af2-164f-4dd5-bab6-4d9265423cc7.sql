-- Phase 1 — Company Profiles schema and linkage
...
-- Indexes and uniqueness (domain and normalized name)
CREATE INDEX IF NOT EXISTS idx_company_profiles_user_id ON public.company_profiles(user_id);
CREATE INDEX IF NOT EXISTS idx_company_profiles_domain ON public.company_profiles (lower(domain));
...
-- Optional unique per user on domain when present (partial unique index)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_class WHERE relname = 'uniq_company_profiles_user_domain'
  ) THEN
    CREATE UNIQUE INDEX uniq_company_profiles_user_domain
