-- Phase 1 — Company Profiles linkage using existing schema (retry 2)
-- Ensure helper columns exist
ALTER TABLE public.company_profiles
  ADD COLUMN IF NOT EXISTS last_enriched_at timestamptz;

-- Create unique index per user on normalized company_name (expression index)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_class WHERE relname = 'uniq_company_profiles_user_norm_company_name_idx'
  ) THEN
    CREATE UNIQUE INDEX uniq_company_profiles_user_norm_company_name_idx
    ON public.company_profiles (user_id, (lower(regexp_replace(company_name, '\\s+', '', 'g'))));
  END IF;
END$$;
