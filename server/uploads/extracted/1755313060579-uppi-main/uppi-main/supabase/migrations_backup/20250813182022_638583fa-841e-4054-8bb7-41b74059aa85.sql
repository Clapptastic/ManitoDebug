...
SELECT id, 1, 'You are a meticulous market analyst. Extract and normalize competitor data for exactly the JSON schema provided. Never invent numbers—return null with low confidence if unknown. Include evidence_sources for each nontrivial claim. Output JSON only.'
...
UPDATE prompts SET current_version_id = (
  SELECT id FROM prompt_versions pv WHERE pv.prompt_id = prompts.id ORDER BY pv.version DESC LIMIT 1
);