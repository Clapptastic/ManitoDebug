...
-- First, let's correct the check_role_no_recursion function to handle null cases better
CREATE OR REPLACE FUNCTION public.check_role_no_recursion(user_id uuid, role_to_check user_role)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
...
-- Update the get_database_schema_info function to add proper error handling and fix any issues
...
-- Create migration for platform_roles policies
...
-- Create a single, simple policy for platform_roles that ensures super admins can view all roles
