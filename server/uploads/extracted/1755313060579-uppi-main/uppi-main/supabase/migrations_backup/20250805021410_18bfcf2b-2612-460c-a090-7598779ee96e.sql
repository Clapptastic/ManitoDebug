-- First, create a function to populate master profiles from competitor analyses
...
BEGIN
  ...
  FOR analysis_record IN 
    ...
    HAVING COUNT(*) >= 1  -- At least 1 analysis
  LOOP
    ...
    IF master_profile_id IS NULL THEN
      ...
      INSERT INTO master_company_profiles (
        ...
        SPLIT_PART(analysis_record.industries, ',', 1), -- Take first industry
        CASE 
          WHEN analysis_record.websites IS NOT NULL 
          THEN REPLACE(REPLACE(analysis_record.websites, 'https://', ''), 'http://', '')
          ELSE NULL 
        END,
        ...
        SPLIT_PART(analysis_record.headquarters_list, ',', 1), -- Take first headquarters
        LEFT(analysis_record.descriptions, 500), -- Limit description length
        ...
        END,
        'pending',
        '00000000-0000-0000-0000-000000000000'::UUID, -- System generated
        '00000000-0000-0000-0000-000000000000'::UUID  -- System generated
      )
