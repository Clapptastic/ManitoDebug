-- Ensure feature_flags exists with flag_key column
DO $$
BEGIN
  IF NOT EXISTS (
  ...
  ELSE
    ...
    ALTER TABLE public.feature_flags ADD COLUMN IF NOT EXISTS value jsonb NOT NULL DEFAULT '{}'::jsonb;
    ALTER TABLE public.feature_flags ADD COLUMN IF NOT EXISTS is_enabled boolean NOT NULL DEFAULT true;
    ...
    -- Create unique index on flag_key if missing
    IF NOT EXISTS (
      SELECT 1 FROM pg_indexes WHERE schemaname='public' AND indexname='feature_flags_flag_key_key'
    ) THEN
      CREATE UNIQUE INDEX feature_flags_flag_key_key ON public.feature_flags(flag_key);
    END IF;
  END IF;
END $$;
...
-- Seed competitor_analysis flag (safe upsert)
...
-- Ensure user_feature_gates exists
