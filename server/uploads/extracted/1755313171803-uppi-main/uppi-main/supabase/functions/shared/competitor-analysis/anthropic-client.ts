...
export async function analyzeWithAnthropic(competitor: string, apiKey: string): Promise<any> {
  // Updated: Enforce full unified dataset schema coverage and require citations
  const prompt = `Analyze the company "${competitor}" and return ONLY valid JSON (no extra text) that matches this COMPLETE schema. Include EVERY field, even if null. If unknown, use null. Provide source_citations with direct URLs for all major claims.
{
  "id": null,
  "user_id": null,
  ...
  try {
    ...
    throw new Error('Failed to parse Anthropic analysis result');
  }
}
