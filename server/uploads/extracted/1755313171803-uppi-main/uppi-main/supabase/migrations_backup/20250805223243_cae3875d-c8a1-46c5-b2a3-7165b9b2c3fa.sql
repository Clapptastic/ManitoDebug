-- Fix the critical RLS policy issue in check-api-keys edge function
-- The function is using service role key but policies are not allowing access

-- First, let's check current api_keys policies
-- Drop and recreate the RLS policies to ensure proper access for edge functions
...
-- Check edge_function_metrics table
...
CREATE POLICY IF NOT EXISTS "Service role can manage function metrics" ON edge_function_metrics
    FOR ALL USING (auth.role() = 'service_role'::text);

-- Create policy for users to view their own metrics
CREATE POLICY IF NOT EXISTS "Users can view their own metrics" ON edge_function_metrics
    FOR SELECT USING (auth.uid() = user_id);

-- Ensure proper logging table access
-- Check if we need to add RLS policies for other tables used by edge functions