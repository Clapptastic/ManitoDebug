-- 1) Feature flags table for global kill-switch
...
DO $$ BEGIN
  ...
  -- Admins and service role can insert
  IF NOT EXISTS (
  ...
  -- Admins and service role can update
  IF NOT EXISTS (
  ...
  -- Admins and service role can delete
  IF NOT EXISTS (
...
-- Seed global flag for competitor analysis (idempotent)
INSERT INTO public.feature_flags (key, description, value, is_enabled)
VALUES ('competitor_analysis', 'Global enable/disable for Competitor Analysis feature', '{"version":"v1"}', true)
ON CONFLICT (key) DO NOTHING;


-- 2) Per-user feature gate table
...
DO $$ BEGIN
  ...
  -- Delete own (admins/service can delete)
  IF NOT EXISTS (
