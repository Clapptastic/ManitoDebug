...
DROP POLICY IF EXISTS "Users manage their own cost limits" ON public.user_cost_limits;
CREATE POLICY "Users manage their own cost limits"
ON public.user_cost_limits
FOR ALL
USING (auth.uid() = user_id)
WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Admins and service role manage all cost limits" ON public.user_cost_limits;
CREATE POLICY "Admins and service role manage all cost limits"
ON public.user_cost_limits
FOR ALL
