-- Phase 1 — Company Profiles schema and linkage (retry with expression indexes)
...
-- Indexes and uniqueness (expression indexes for lower(domain))
CREATE INDEX IF NOT EXISTS idx_company_profiles_user_id ON public.company_profiles(user_id);
CREATE INDEX IF NOT EXISTS idx_company_profiles_domain_expr ON public.company_profiles ((lower("domain")));

-- Unique per user on normalized name (strip spaces, lower)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'uniq_company_profiles_user_norm_name'
  ) THEN
    ALTER TABLE public.company_profiles
    ADD CONSTRAINT uniq_company_profiles_user_norm_name
    UNIQUE (user_id, (lower(regexp_replace(name, '\\s+', '', 'g'))));
  END IF;
END$$;

-- Optional unique per user on domain when present (partial unique index using expression)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_class WHERE relname = 'uniq_company_profiles_user_domain_expr'
  ) THEN
    CREATE UNIQUE INDEX uniq_company_profiles_user_domain_expr
    ON public.company_profiles (user_id, (lower("domain")))
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_company_profiles_updated_at'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'Users manage their own company profiles'
  ) THEN
...
BEGIN
  ...
  -- Try match by domain first when present
  IF domain_param IS NOT NULL AND domain_param <> '' THEN
    SELECT id INTO existing_id
    FROM public.company_profiles
    WHERE user_id = user_id_param AND lower("domain") = lower(domain_param)
    LIMIT 1;
  END IF;

  -- Fallback to normalized name
  IF existing_id IS NULL AND norm_name <> '' THEN
    SELECT id INTO existing_id
    FROM public.company_profiles
    WHERE user_id = user_id_param AND lower(regexp_replace(name, '\\s+', '', 'g')) = norm_name
    LIMIT 1;
  END IF;

  IF existing_id IS NULL THEN
    INSERT INTO public.company_profiles (user_id, name, "domain", website_url, description, profile_data, last_enriched_at)
    VALUES (user_id_param, COALESCE(name_param, ''), domain_param, website_url_param, NULL, COALESCE(profile_data_param, '{}'::jsonb), now())
    RETURNING id INTO existing_id;
  ELSE
    UPDATE public.company_profiles
    SET 
      name = COALESCE(name_param, name),
      "domain" = COALESCE(domain_param, "domain"),
      website_url = COALESCE(website_url_param, website_url),
      profile_data = COALESCE(profile_data, '{}'::jsonb) || COALESCE(profile_data_param, '{}'::jsonb),
