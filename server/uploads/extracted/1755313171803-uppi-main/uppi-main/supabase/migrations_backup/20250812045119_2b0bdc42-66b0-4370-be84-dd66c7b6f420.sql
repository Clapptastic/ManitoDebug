...
create policy "Users can insert their own flow test runs"
  on public.flow_test_runs for insert
  with check (auth.uid() = user_id);

create policy "Users can view their own flow test runs"
  on public.flow_test_runs for select
  using (auth.uid() = user_id);

-- Optional update/delete
create policy "Users can update their own flow test runs"
  on public.flow_test_runs for update
  using (auth.uid() = user_id) with check (auth.uid() = user_id);

create policy "Users can delete their own flow test runs"
