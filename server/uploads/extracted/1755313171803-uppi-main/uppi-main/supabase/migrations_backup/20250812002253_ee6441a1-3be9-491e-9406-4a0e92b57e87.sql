-- Resilient migration to fix Supabase errors without assuming column 'name' exists

-- 1) api_keys policies/grants
DO $$
BEGIN
...
-- 2) audit_logs permit inserts from authenticated
DO $$
BEGIN
...
-- 3) feature_flags: add flag_key; backfill from flag_name if present; index + grant
DO $$
DECLARE
  has_flag_name boolean := false;
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'feature_flags') THEN
    ...
    SELECT EXISTS (
      SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'feature_flags' AND column_name = 'flag_name'
    ) INTO has_flag_name;

    IF has_flag_name THEN
      EXECUTE 'UPDATE public.feature_flags SET flag_key = COALESCE(flag_key, flag_name) WHERE flag_key IS NULL';
    END IF;
...
-- 4) user_feature_gates table and policies
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'user_feature_gates') THEN
