-- Retry migration with policy creation wrapped to avoid IF NOT EXISTS

-- 1) Enhance competitor_analyses with new tracking fields (idempotent)
...
DO $$ BEGIN
  CREATE POLICY "Users manage news for their analyses"
...
DO $$ BEGIN
  CREATE POLICY "Service role full access - news_articles"
    ON public.news_articles
    FOR ALL
...
DO $$ BEGIN
  CREATE POLICY "Users manage their drill down sessions"
...
DO $$ BEGIN
  CREATE POLICY "Service role full access - ai_drill_down_sessions"
    ON public.ai_drill_down_sessions
    FOR ALL
    USING (auth.role() = 'service_role')
    WITH CHECK (auth.role() = 'service_role');
...
DO $$ BEGIN
  CREATE POLICY "Users manage metrics for their analyses"
...
DO $$ BEGIN
  CREATE POLICY "Service role full access - data_quality_metrics"
    ON public.data_quality_metrics
    FOR ALL
    USING (auth.role() = 'service_role')
    WITH CHECK (auth.role() = 'service_role');
