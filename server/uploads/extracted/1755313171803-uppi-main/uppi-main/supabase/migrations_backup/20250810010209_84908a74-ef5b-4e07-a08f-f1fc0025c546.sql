-- Extend affiliate_programs with domain for matching (idempotent)
ALTER TABLE public.affiliate_programs
  ADD COLUMN IF NOT EXISTS domain text;

-- Create affiliate_link_suggestions to log external links and potential affiliate programs (idempotent)
...
-- Indexes for fast admin review (idempotent)
...
-- Separate update and delete policies to avoid syntax errors
DROP POLICY IF EXISTS "Admins can update suggestions" ON public.affiliate_link_suggestions;
...
DROP POLICY IF EXISTS "Admins can delete suggestions" ON public.affiliate_link_suggestions;
