-- Fix the ambiguous column reference in manage_api_key function
CREATE OR REPLACE FUNCTION public.manage_api_key(operation text, user_id_param uuid DEFAULT NULL::uuid, provider_param text DEFAULT NULL::text, api_key_param text DEFAULT NULL::text, key_hash_param text DEFAULT NULL::text, masked_key_param text DEFAULT NULL::text, key_prefix_param text DEFAULT NULL::text, api_key_id_param uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  CASE operation
    WHEN 'insert' THEN
      -- Check for existing key - use table alias to avoid ambiguity
      SELECT ak.id, ak.vault_secret_id INTO existing_key_id, existing_secret_id
      ...
      -- Upsert API key record with explicit column references
      ...
      ON CONFLICT (user_id, provider) DO UPDATE SET
        ...
        is_active = true,
        status = 'active',
        vault_secret_id = CASE WHEN use_vault THEN new_secret_id ELSE api_keys.vault_secret_id END,
        updated_at = now()
    ...
    WHEN 'select' THEN
      SELECT jsonb_agg(
        jsonb_build_object(
          'id', ak.id, 'provider', ak.provider, 'name', ak.name, 'masked_key', ak.masked_key,
          'status', ak.status, 'is_active', ak.is_active, 'created_at', ak.created_at,
          'updated_at', ak.updated_at, 'last_used_at', ak.last_used_at, 'permissions', ak.permissions
        )
      ) INTO result
      FROM public.api_keys ak
    ...
    WHEN 'delete' THEN
      ...
      WHERE ak.id = api_key_id_param AND ak.user_id = user_id_param;
      result := jsonb_build_object('success', true, 'deleted_id', api_key_id_param);

    WHEN 'get_for_decryption' THEN
      SELECT ak.id, ak.provider, ak.vault_secret_id, ak.api_key, ak.status
      INTO existing_key_id, v_provider, existing_secret_id, secret_value, v_status
      FROM public.api_keys ak
      WHERE ak.user_id = user_id_param AND ak.provider = provider_param AND ak.is_active = true
      LIMIT 1;
      ...
      IF use_vault AND existing_secret_id IS NOT NULL THEN
        ...
        EXCEPTION WHEN OTHERS THEN
          ...
          SELECT ak.api_key INTO secret_value FROM public.api_keys ak WHERE ak.id = existing_key_id;
        END;
      END IF;
