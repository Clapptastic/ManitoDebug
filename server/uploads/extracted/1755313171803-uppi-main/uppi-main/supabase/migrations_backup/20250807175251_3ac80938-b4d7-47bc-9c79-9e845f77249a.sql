-- Create support system tables for Phase 11
...
CREATE TABLE public.support_tickets (
    ...
    category TEXT NOT NULL CHECK (category IN ('billing', 'technical', 'feature_request', 'bug_report', 'general')),
...
CREATE POLICY "Support team can update tickets" ON public.support_tickets
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM public.support_team_members 
            WHERE user_id = auth.uid() AND is_active = true
...
CREATE POLICY "Users can create messages on their tickets" ON public.support_ticket_messages
...
CREATE POLICY "Support team can create messages" ON public.support_ticket_messages
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM public.support_team_members 
            WHERE user_id = auth.uid() AND is_active = true
...
CREATE INDEX idx_knowledge_base_articles_published ON public.knowledge_base_articles(is_published);
CREATE INDEX idx_knowledge_base_feedback_article_id ON public.knowledge_base_feedback(article_id);
