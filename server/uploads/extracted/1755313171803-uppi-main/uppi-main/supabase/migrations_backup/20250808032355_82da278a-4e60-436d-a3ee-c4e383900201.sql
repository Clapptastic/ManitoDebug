-- Idempotent migration re-run with policy guards
...
-- Ensure trigger (drop/create for idempotency)
...
begin
  if not exists (
    select 1 from pg_policies p where p.schemaname='public' and p.tablename='provider_preferences' and p.policyname='Users manage own provider preferences'
  ) then
    create policy "Users manage own provider preferences"
...
begin
  if not exists (
    select 1 from pg_policies p where p.schemaname='public' and p.tablename='provider_preferences' and p.policyname='Org admins manage org provider preferences'
  ) then
    create policy "Org admins manage org provider preferences"
      on public.provider_preferences
      for all
...
begin
  if not exists (
    select 1 from pg_policies p where p.schemaname='public' and p.tablename='provider_preferences' and p.policyname='Service role full access - provider preferences'
  ) then
    create policy "Service role full access - provider preferences"
      on public.provider_preferences
      for all
...
-- api_usage_costs: org column and indexes
...
-- Org admin SELECT policy (guarded)
do $$
begin
  if not exists (
    select 1 from pg_policies p where p.schemaname='public' and p.tablename='api_usage_costs' and p.policyname='Org admins can view org api usage costs'
  ) then
    create policy "Org admins can view org api usage costs"
...
-- Trigger to populate organization_id on insert
...
-- Nightly cron: unschedule then schedule
select cron.unschedule('invoke-bulk-consolidate-companies-nightly');
