-- Update manage_api_key function to support delete operation
CREATE OR REPLACE FUNCTION manage_api_key(
  operation text,
  user_id_param uuid,
...
BEGIN
  ...
  CASE operation
    ...
    WHEN 'select' THEN
      -- Get all API keys for user without ORDER BY to avoid GROUP BY issues
      SELECT jsonb_agg(
        jsonb_build_object(
          'id', id,
          'provider', provider,
